...
export default {
  ...
  methods: {
    submitForm() {
      axios.post('http://localhost:5035/api/v1/-carts', this.form)
          .then(response => {
          ...
          .catch(error => {
            console.error(error);
            // Aquí puedes agregar la lógica para manejar los errores
          });
    },

    submitProductCartForm() {
      axios.post('http://localhost:5035/api/v1/-products-cart', this.productCartForm)
          .then(response => {
            console.log(response);
            // Aquí puedes agregar la lógica para manejar la respuesta del servidor
          })
